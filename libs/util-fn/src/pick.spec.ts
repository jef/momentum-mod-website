...
describe('pickDeep', () => {
  it('returns an object with only the specified deeply nested key', () => {
    const obj = { a: 1, b: { c: 2, d: 3 } };
    const result = pickDeep(obj, 'b.d');
    expect(result).toEqual({ b: { d: 3 } });
  });

  it('returns an object with only the specified deeply nested keyswhen an array of keys is provided', () => {
    const obj = { a: 1, b: { c: 2, d: 3 } };
